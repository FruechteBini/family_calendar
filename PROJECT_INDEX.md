...
| **Flutter AI** | KI-Essensplan Wizard, Voice FAB + Sprachbefehl-Sheet (`_VoiceCommandSheet`), Rezept-Kategorisierung (`categorizeRecipes` / `applyRecipeCategorization`) | `features/ai/domain/ai_models.dart`, `features/ai/data/ai_repository.dart`, `features/ai/presentation/ai_meal_plan_wizard.dart`, `app/app_shell.dart`, `core/speech/speech_service.dart`, `core/speech/voice_state.dart` | Auth, Rezepte, speech_to_text, permission_handler |
...
| Flutter Sprachbefehle | `app/app_shell.dart` (`_FamilienherdVoiceFAB`, `_VoiceCommandSheet`), `core/speech/speech_service.dart`, `core/speech/voice_state.dart`, `features/ai/data/ai_repository.dart` |
